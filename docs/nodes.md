---
template: base_no_sidebar.html
---
# Nodes
<div style="padding: 10px">
<div id="node-map-canvas" style="width: 100%; height: 40vh"></div>
</div>

Below are nodes that have an average SNR of ≥ -20dB or more (over a 24hr period). Data for this table is pulled from [here](https://malla.nashme.sh/traceroute-graph).

Infrastructure nodes are shaded as <span style="color: steelblue">blue</span>.

<div id="table-nodes"></div>